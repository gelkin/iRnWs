...
from search_backend import prepare_arguments
...
def get_snippet_similarity(snippet_one_words: set, snippet_two_words: set):
    ...
    return len(sim_words) / (math.log(len(snippet_one_words)) + math.log(len(snippet_two_words)))
...
def split_text_into_paragraphs(text):
    ...
    for paragraph in text.splitlines():
        if paragraph:
            paragraph_words = set(prepare_arguments(paragraph))
            if paragraph_words:
